...
`android/` is a Gradle project of its own, outside the pnpm workspace, and
nothing in it reaches the npm package. You need JDK 17 and the Android SDK
with platform 36 and build-tools 35 (Android Studio brings both). From
`android/`:

```bash
./gradlew :app:testDebugUnitTest   # the tests, on the JVM
./gradlew :app:lintDebug            # Android's lint; the CI fails on its errors
./gradlew :app:assembleDebug       # app/build/outputs/apk/debug/app-debug.apk
```

The debug build installs as a separate app (its id ends in `.dev`), next to a
published one. To try it end to end you need a computer running Agent
Workbench with remote access on and its SSH server running, and the phone or
the emulator on its network.

Three rules on top of the ones above:

- **No Google services, Firebase, analytics or crash reporting.** The app
  talks only to the user's computer.
- **The app's texts go in the nine languages**, in
  `app/src/main/res/values*/strings.xml`, English first, like the
  interface's.
- **The web and the app know each other through three things**: the
  `AgentWorkbenchAndroid/` user-agent token, `window.agentWorkbenchBack()` and
  the `agentworkbench://settings` link
  ([`ARCHITECTURE.md`](ARCHITECTURE.md#7-the-android-app)). A change to one
